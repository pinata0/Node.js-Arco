`Node.js`와 `NodeJS`는 **사실상 같은 것을 가리킨다.**

공식 명칭은 **Node.js**이고, `NodeJS`나 `nodejs`는 사람들이 편의상 붙여 쓰는 비공식 표기야.
...
| `Node.js` | 공식 명칭 | ✅ |
| `NodeJS` | 비공식 표기 | △ |
| `nodejs` | 패키지명·폴더명·명령어 등에서 종종 사용 | △ |
| `Node` | 문맥상 줄여 부르는 표현 | ✅ |

즉 기술 문서나 README에는 **Node.js**라고 쓰는 게 가장 좋다.
